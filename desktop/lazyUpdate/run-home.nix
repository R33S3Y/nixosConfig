      ...
      <svg
        ...
        <defs
          ...
          <linearGradient
            ...
            <stop
              style="stop-color:#${config.stylix.base16Scheme.base06};stop-opacity:1"
              offset="0"
              id="stop5564" />
            <stop
              id="stop5566"
          ...
          <linearGradient
            ...
            <stop
              style="stop-color:#${config.stylix.base16Scheme.base04};stop-opacity:1"
              offset="0"
              id="stop5055" />
            <stop
              id="stop5057"
...
in {
  home.activation.makeLazyUpdatesFiles = lib.mkAfter ''
    ...
    if [ -f ${config.home.homeDirectory}/.config/hypr/nixSnowflake.svg ]; then
      rm ${config.home.homeDirectory}/.config/hypr/nixSnowflake.svg
    fi
    cp ${nixSnowflake} ${config.home.homeDirectory}/.config/hypr/nixSnowflake.svg
    chmod 755 ${config.home.homeDirectory}/.config/hypr/nixSnowflake.svg

    mkdir -p ${config.home.homeDirectory}/.config/hypr/
    if [ -f ${config.home.homeDirectory}/.config/hypr/lazyUpdate.sh ]; then
      rm ${config.home.homeDirectory}/.config/hypr/lazyUpdate.sh
    fi
    cp ${lazyUpdate} ${config.home.homeDirectory}/.config/hypr/lazyUpdate.sh
    chmod 755 ${config.home.homeDirectory}/.config/hypr/lazyUpdate.sh
  '';

  wayland.windowManager.hyprland.settings.exec-once = [
    "bash ~/.config/hypr/lazyUpdate.sh"
  ];
}